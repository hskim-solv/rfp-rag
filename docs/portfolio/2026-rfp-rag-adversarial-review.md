...
as a senior `AI Agent Engineer` portfolio, with `LLM/RAG AI Engineer` and
`AI Agent Backend Engineer` overlap. The reviewer was constrained to repository
evidence only and was forbidden from editing files, running paid/API commands,
exposing raw RFP text, or dumping artifacts.
...
|---|---|---|---|---|
| F-01 | blocker: latest source-first real semantic gate is missing | reopened under v6 | `artifacts/index_real/manifest.json` has parsed-source lineage from the v5 refresh, but `rfp-rag-real-v6` now requires cross-document hard floors and model/prompt lineage. Existing v5 evidence must fail closed. | Regenerate/improve the real lane before using `rag_quality_complete=true` as a current claim. |
...
| F-04 | major: evidence UX/service surface is missing | accept | final goal requires a dashboard/service, but repo has no `app/`, `server/`, `dashboard/`, `Dockerfile`, FastAPI, or Streamlit dependency | Keep service/dashboard as a senior-ready milestone, not a completed claim. Require answer, citation, chunk, source preview, agent state/tool trace, HITL/checkpoint status, gate, failure, latency, and cost visibility. |
...
3. **Keep the real gate fresh on the parsed-source index.**
   ...
   `text_source=parsed` and `parse_manifest_path`, but the old contract was
   `rfp-rag-real-v5`. Future current claims require `rfp-rag-real-v6` evidence.

4. **Make evidence inspectable.**
   A small service/dashboard must show answer, citations, retrieved chunks,
   source preview, agent state transitions, tool calls, HITL/checkpoint status,
   failure reason, gate freshness, latency, token/cost estimate, and trace
   identifiers. This is a portfolio evidence surface, not a chatbot UI.

5. **Keep the AI agent claim evidence-backed.**
   Agent workflow, HITL, visual sidecar, and MCP/FastMCP work are valuable only
   after source/retrieval quality and failure visibility are credible; the final
   AI Agent Engineer claim must show what the agent decides, which tools it may
   call, how loops are bounded, and how failures are inspected.
