...
평가(evaluation), 관찰성(observability), 보안(guardrails), public-safe hosted
reviewer demo를 gate로 검증하는 Agentic RAG backend입니다.
...
| 운영 경계 | Docker/CI와 public-safe hosted reviewer demo를 검증 대상으로 둠. full hosted production SaaS, public dashboard, live SLO는 아직 claim하지 않음 |
...
- **Hosted reviewer demo wording:** public-safe reviewer URL과
  local/container 재현성을 claim으로 두되, full hosted production SaaS나
  live-traffic SLO처럼 과장하지 않습니다.
...
5. 마지막에 non-claim을 명확히 말합니다: full hosted production SaaS, public dashboard, provider billing telemetry, live-traffic SLO, reranker quality win은 아직 별도 승인/증거가 필요합니다.

## 현재 한계와 다음 단계

현재 레포의 목표는 시니어 포트폴리오용 public-safe hosted reviewer demo입니다. 이
claim은 HTTPS reviewer URL, reviewer-token boundary, rate-limit, public-safe source
boundary, redacted logs/metrics, rollback evidence가 모두 통과할 때만 열립니다. 그
너머의 full SaaS account/billing/admin, public dashboard, provider billing telemetry,
live-traffic SLO는 비용, credential, public disclosure가 걸리므로 별도 승인 후
별도 phase로 진행해야 합니다.
