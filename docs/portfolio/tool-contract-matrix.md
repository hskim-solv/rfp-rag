...
|---|---|---|---|---|---|---|
| `GET /healthz` | no input; `{"ok": true, "service": "rfp-rag", "git_sha": ...}` output | read-only | public health check in local and hosted reviewer mode; `git_sha` must match deployed revision for hosted evidence | hosted ingress/runtime timeout | no sensitive payload | HTTP status |
...
| `GET /v1/gates` | hosted demo returns `public_demo_gate=true`; local mode accepts optional `root` query param constrained to repo root and returns gate status output | read-only artifact inspection or public demo gate | hosted reviewer mode requires reviewer token; public demo gate does not read private artifacts | local full gate path is required by `artifacts/service_ops/summary.json`; hosted smoke requires `overall_ok=true` and `public_demo_gate=true` | aggregate gate fields only; no raw RFP source | HTTP status |
