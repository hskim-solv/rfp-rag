...
def generate_abstention_questions() -> list[dict[str, Any]]:
    questions = [
        ...
        "이 RFP 모음에 없는 스마트팜 위성관제 사업의 제안 등급은?",
        "이 RFP 모음에 없는 해저 데이터센터 사업의 평가 감점 항목은?",
        "이 RFP 모음에 없는 드론 택배 항로 사업의 계약 위험도는?",
        "이 RFP 모음에 없는 메타버스 법정 구축 사업의 요구 우선순위는?",
        "이 RFP 모음에 없는 우주 엘리베이터 사업의 제안 발표 순서는?",
        "이 RFP 모음에 없는 극지 연구 로봇 사업의 평가 시뮬레이션 값은?",
        "이 RFP 모음에 없는 AI 판사 보조 사업의 계약 분쟁 점수는?",
        "이 RFP 모음에 없는 해양 탄소포집 사업의 제안 리허설 결과는?",
        "이 RFP 모음에 없는 재난 예측 위성 사업의 요구 변경 로그는?",
        "이 RFP 모음에 없는 도시 항공교통 사업의 평가 보정 계수는?",
        "이 RFP 모음에 없는 디지털 트윈 항만 사업의 계약 협상 순위는?",
        "이 RFP 모음에 없는 공공 로봇 배달 사업의 제안 심의 메모는?",
        "이 RFP 모음에 없는 초전도 전력망 사업의 요구 검토 결과는?",
        "이 RFP 모음에 없는 자율운항 선박 사업의 평가 리스크 표는?",
        "이 RFP 모음에 없는 공공 블록체인 신원 사업의 계약 쟁점은?",
        "이 RFP 모음에 없는 지능형 교통 신호 사업의 제안 순위 변동은?",
        "이 RFP 모음에 없는 산림 화재 예측 사업의 요구 누락 항목은?",
        "이 RFP 모음에 없는 의료 데이터 중개 사업의 평가 내부 의견은?",
        "이 RFP 모음에 없는 문화재 XR 복원 사업의 계약 검토 메모는?",
        "이 RFP 모음에 없는 국방 양자센서 사업의 제안 보완 요청은?",
    ]
    return [
        {
