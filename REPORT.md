...
최종 품질 목표는 단일 평균 점수가 아니라 gate 조합으로 정의한다. 최종 산출물은 단순 RAG demo가 아니라 AI Agent Engineer 시니어 포트폴리오로서 source-first retrieval 품질, LangGraph typed state/conditional routing, bounded retry/reflection, tool allowlist, HITL/checkpoint, streaming service, tracing/latency/cost, guardrail, CI evidence가 모두 artifact로 검증되어야 한다. 현재 저장소는 production-grade backend shape와 credential-free 회귀 증거를 갖췄지만, 공개용 senior-ready product claim은 `rfp-rag-real-v6` real gate와 Stage 2 독립 holdout/security/ops/cost gate가 통과할 때까지 보류한다. Offline lane은 credential-free 회귀(regression)/스캐폴드(scaffold) 검증이고, semantic RAG 품질은 cross-document hard floor와 model/prompt lineage를 포함한 parsed-source `real_openai` lane이 담당한다.

| 목표 | 판정 기준 | 현재 상태 |
|---|---|---|
| Offline 회귀 안전성 | `offline_rag.offline_scaffold_complete == true` | 통과 |
| 현재 semantic RAG 품질 | `rfp-rag-real-v6`: `real_rag.rag_quality_complete == true`, cross-document hard floor, model/prompt lineage | 아직 주장하지 않음 |
...
> `evaluation_valid=true`; **v6 상태:** not yet claimed because the
> cross-document hard slice and model/prompt lineage gate were added later.
...
with `real_openai`. It is now historical evidence: v6 adds cross-document
hard-slice floors and model/prompt lineage, so this run cannot be used as a
current final quality claim.
...
| real_rag | `false` | stale v5 contract / v6 cross-document hard floor not yet passed |

This is no longer a complete current gate state after the v6 hardening. The
approved `real_openai` refresh remains useful evidence, but `gate_status` should
report `overall_ok=false` until `artifacts/eval_real` is regenerated under
`rfp-rag-real-v6` and passes the cross-document hard floor.
...
- `python3 -m rfp_rag.gate_status` reports `overall_ok=false` until v6 real evidence passes;
