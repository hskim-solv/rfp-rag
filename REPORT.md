...
이번 closeout 환경에서는 open lane 산출물을 만들지 않았다. 로컬 Ollama
`http://localhost:11434/api/tags`는 응답했지만 모델 목록이 비어 있어 기본 open embedding
모델인 `bge-m3`가 설치되어 있지 않았고, DeepSeek 원격 생성에 필요한
`RFP_OPEN_API_KEY` 또는 `DEEPSEEK_API_KEY`도 설정되어 있지 않았다.

따라서 `artifacts/eval_open`은 이 closeout 환경에서 unavailable/not produced 상태로
남는다. 첫 성공 런이 `score_distribution`에 in-domain top score와 abstention top score의
분리 가능한 분포를 남기기 전까지 open lane의 `min_score`는 `미확정`이다.

향후 실험은 `bge-m3`와 생성 백엔드 키 또는 로컬 생성 백엔드가 준비된 뒤에만 이 레인을
baseline으로 사용한다. 최종 품질 주장은 계속 `artifacts/eval_real` /
