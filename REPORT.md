...
source-first parsed index, `rfp-rag-real-v5`, 545-query benchmark의 현재 품질
증거로 사용하지 않는다.
...
> **최종 gate 상태:** `rag_quality_complete=true`, `thresholds_met=true`,
> `evaluation_valid=true`

The latest source-first HWP/PDF real semantic gate passed after regenerating the
canonical parsed-source real index and rerunning the 545-query benchmark with
`real_openai`.
...
current real contract인 `artifacts/eval_real` / `rfp-rag-real-v5`에서만 판정한다.
...
`artifacts/eval_real` / `rfp-rag-real-v5` 증거에만 귀속된다.
...
index / `rfp-rag-real-v5` / 545-query benchmark 증거가 아니므로 현재 final 품질
claim에는 사용하지 않는다.
...
1. **Latest source-first real gate was missing and is now resolved.** The old
   ...
   `rag_quality_complete=true` under `rfp-rag-real-v5`.
...
5. **Gate status was too shallow.** `rfp_rag.gate_status` now validates contract
   ...
   approved source-first real refresh, the current local status is
   `overall_ok=true`.
...
This is the current local source-first gate state after the approved
`real_openai` refresh. `gate_status` reports `overall_ok=true`.
