...
| 총합 | 515 | offline contract 검증 |

## 6. Offline 평가 결과

| 지표 | 결과 |
|---|---:|
| Recall@3 | 0.9824742268041237 |
| Recall@5 | 0.9855670103092784 |
| All expected docs retrieved@5 | 0.9731958762886598 |
| MRR | 0.9845360824742269 |
| Citation presence | 0.9938144329896907 |
| Citation validity | 0.9917525773195877 |
| Metadata exact match | 0.945 |
...
current real contract인 `artifacts/eval_real` / `rfp-rag-real-v4`에서만 판정한다.
...
`artifacts/eval_real` / `rfp-rag-real-v4` 증거에만 귀속된다.
...
- offline contract: `rfp-rag-offline-v3`
- real contract: `rfp-rag-real-v4`
- open contract: `rfp-rag-open-v3`
...
| expected_contract_version | `rfp-rag-offline-v3` |
| offline_scaffold_complete | `true` |
| rag_quality_complete | `false` |
| ok | `true` |
| metric_warnings | `[]` |
...
3. Benchmark hardening: 100-document metadata coverage, 30 hard-negative
   abstention questions, 30 section-labeled lookup questions, and 20
   cross-document comparison questions are now the default offline query
   contract; next expand into paraphrases and table/visual slices.
...
2. **Evaluation set is still incomplete for the final claim.** The current
   ...
   controls, 30 section-labeled lookup questions, 20 cross-document comparison
   questions, and 25 reviewed visual/table questions
   (`query_set_counts.total=515`). It still lacks the required paraphrase slice.
   The existing real run is also stale and must not be used as the final
   benchmark until explicitly rerun with the current contract.
...
| query_set_counts.total | `515` |
| recall@5 | `0.9855670103092784` |
| all_expected_docs_retrieved@5 | `0.9731958762886598` |
...
| citation_validity | `0.9917525773195877` |
| section_hit_rate | `1.0` |
