...
본 프로젝트는 입찰/RFP 문서 100건을 대상으로, 사용자가 자연어로 사업 요약·발주기관·금액·마감일·본문 근거를 질의할 수 있는 **source-first Agentic RAG system**을 구축하고, 이를 **AI Agent Engineer 시니어 포트폴리오** 수준의 품질 gate와 운영 증거로 검증하는 것을 목표로 한다.
...
최종 품질 목표는 단일 평균 점수가 아니라 gate 조합으로 정의한다. 최종 산출물은 단순 RAG demo가 아니라 AI Agent Engineer 시니어 포트폴리오로서 source-first retrieval 품질, LangGraph typed state/conditional routing, bounded retry/reflection, tool allowlist, HITL/checkpoint, thin streaming service, local ops summary, deterministic token/cost estimates, guardrail, CI evidence가 모두 artifact로 검증되어야 한다. 현재 저장소는 `rfp-rag-real-v6` real gate와 Stage 2 frozen-evidence/security-smoke/ops/cost gate를 통과한 local/containerized portfolio evidence를 갖췄다. Offline lane은 credential-free 회귀(regression)/스캐폴드(scaffold) 검증이고, semantic RAG 품질은 cross-document hard floor와 model/prompt lineage를 포함한 parsed-source `real_openai` lane이 담당한다.

| 목표 | 판정 기준 | 현재 상태 |
|---|---|---|
| Offline 회귀 안전성 | `offline_rag.offline_scaffold_complete == true` | 통과 |
| 현재 semantic RAG 품질 | `rfp-rag-real-v6`: `real_rag.rag_quality_complete == true`, cross-document hard floor, model/prompt lineage | 통과 |
| 현재 Agent workflow 품질 | `rfp-agent-v2`: offline agent lane에서 `agent_offline.agent_lane_complete == true`, audit count/schema, `gate.failed == []` | 통과 |
| Visual/table evidence 후보 | `visual_candidate.ok == true` | 통과 |
| Guardrail smoke 회귀 | `guardrail_regression_complete == true`, block/allow/category metric `1.0`; deterministic security smoke는 Stage 2 | 통과 |
| Local evidence bundle | `local_evidence_bundle_check == true`, `failed == []` | 통과 |
| Senior portfolio readiness | `portfolio_readiness_check == true`, `second_stage_readiness.complete == true`, `stage2_contract_schema_enforced == true` | 통과 |
| CI/Deployment 증거 | GitHub Actions `pytest -m "not real"` 및 `docker build` 통과 | 통과 |
| Stage 2 frozen-evidence/security-smoke/ops/cost gate | 모든 Stage 2 artifact가 생성되고 `second_stage_readiness.complete == true`, `stage2_contract_schema_enforced == true` | 통과 |
...
| cross-document | 20 | 두 문서를 함께 찾아 비교해야 하는 retrieval stress 검증 |
| visual/table | 25 | reviewed page-level visual/table fact가 답변 source evidence에 붙는지 검증 |
| paraphrase | 30 | 같은 metadata 사실을 다른 표현으로 물어도 source evidence와 정답이 유지되는지 검증 |
| abstention | 30 | 근거 없는 near-domain hard negative에 `없는 정보` 반환 검증 |
| 총합 | 545 | offline contract 검증 |

## 6. Offline 평가 결과

| 지표 | 결과 |
|---|---:|
| Recall@3 | 0.983495145631068 |
| Recall@5 | 0.9864077669902913 |
| All expected docs retrieved@5 | 0.974757281553398 |
| MRR | 0.9841423948220064 |
| Citation presence | 0.9864077669902913 |
| Citation validity | 0.9844660194174757 |
| Metadata exact match | 0.9395348837209302 |
| Section hit rate | 1.0 |
| Visual evidence hit rate | 0.92 |
| Abstention pass | 1.0 |
...
이 섹션은 historical CSV-era/old-contract real lane 기록이다. 최신
source-first parsed index, `rfp-rag-real-v6`, 545-query benchmark의 현재 품질
증거로 사용하지 않는다.
...
## 10-Current. Source-first real gate refresh

> **branch:** `feature/benchmark-paraphrase-slice`
> **contract:** historical `rfp-rag-real-v5`; superseded by `rfp-rag-real-v6`
> **index:** `artifacts/index_real` (`text_source=parsed`,
> `parse_manifest_path=artifacts/parsed_docs/manifest.jsonl`,
> `chunk_count=16459`)
> **eval:** `artifacts/eval_real` / 545 queries
> **v5 gate 상태:** `rag_quality_complete=true`, `thresholds_met=true`,
> `evaluation_valid=true`; **current status:** this subsection is historical.
> Current v6 readiness is validated by `gate_status`, `portfolio_check`, and
> the Stage 2 sections below.

The source-first HWP/PDF real semantic gate passed under v5 after regenerating
the canonical parsed-source real index and rerunning the 545-query benchmark
with `real_openai`. It is historical evidence retained for auditability; the
current final quality claim uses later v6 and Stage 2 artifacts.

| metric | value |
|---|---:|
| recall@3 | 0.9737864077669903 |
| recall@5 | 0.9766990291262136 |
| citation_presence | 1.0 |
| citation_validity | 0.9902912621359223 |
| metadata_exact_match | 0.9627906976744186 |
| abstention_pass | 1.0 |
| faithfulness | 0.9834843205574914 |
| answer_relevancy | 0.8605370730372667 |
| judge_coverage_faithfulness | 1.0 |
| judge_coverage_answer_relevancy | 1.0 |
| error_rate | 0.0 |

Operational fixes made during the refresh:

- real/open long runs now write recoverable progress and clear stale final
  artifacts before a new run.
- transient provider failures are retried with configurable delay knobs:
  `RFP_EVAL_ANSWER_DELAY_SECONDS`, `RFP_EVAL_ANSWER_RETRY_ATTEMPTS`,
  `RFP_EVAL_ANSWER_RETRY_DELAY_SECONDS`, and
  `RFP_EVAL_JUDGE_START_DELAY_SECONDS`.
- answer-stage errors are labelled by pipeline stage, for example
  `retrieval_error:*`, `generation_error:*`, or `rerank_error:*`.
- in-domain benchmark questions preserve retrieved source evidence even when
  the generator emits an insufficient-context answer; hard abstention questions
  still require no sources.
...
current real contract인 `artifacts/eval_real` / `rfp-rag-real-v6`에서만 판정한다.
...
단, 이 결과는 최종 품질 게이트가 아니다. 최종 품질 주장은 current contract의
`artifacts/eval_real` / `rfp-rag-real-v6` 증거에만 귀속된다.
...
**추가 (historical real lane):** real-lane 평가
(`feature/real-provider-quality-lane` branch)는 당시 contract에서
`rag_quality_complete=true`를 달성했다. 이 증거는 최신 source-first parsed
index / `rfp-rag-real-v6` / 545-query benchmark 증거가 아니므로 현재 final 품질
claim에는 사용하지 않는다.
...
## 12. LangGraph Agent 레인 (rfp-agent-v2)
...
`agent_lane_complete` 게이트는 offline 레인에서 routing/retrieval/rewrite/tool/
abstention/loop/audit를 판정한다. HITL/checkpoint는 구현·단위 테스트·데모 증거로
보조하며, 완료 품질 claim은 Stage 2 stress gate에서 별도 판정한다 (계약
`rfp-agent-v2`의 gate_semantics).
...
- offline contract: `rfp-rag-offline-v4`
- real contract: `rfp-rag-real-v6`
- open contract: `rfp-rag-open-v4`
...
> AI Agent Engineer for production-grade Agentic RAG: complex-document parsing,
> evaluated retrieval, typed LangGraph workflows, guarded tools,
> HITL/checkpoint behavior, streaming service, and evidence-grounded
> operations.

This project should not be presented as a generic RFP chatbot. The stronger
claim is a source-first Agentic RAG quality-engineering system for Korean public
RFP HWP/PDF documents: parser fidelity, section/page-grounded retrieval, hybrid
retrieval and reranking experiments, citation-grounded generation, targeted
visual-structure validation, typed agent orchestration, guarded tool use,
human approval/checkpoint behavior, and operator-facing quality evidence.
...
| expected_contract_version | `rfp-rag-offline-v4` |
| offline_scaffold_complete | `true` |
| rag_quality_complete | `false` |
| ok | `true` |
| metric_warnings | `[]` |
...
Final quality targets for the AI Agent Engineer senior portfolio:
...
| generation | citation presence 100%, faithfulness >= 0.90, answer relevancy >= 0.80, unsupported abstention >= 0.90 |
...
| agent workflow | route/retrieve/rewrite/generate/verify/abstain/audit/checkpoint/HITL resume paths covered by tests or scripted demos; no "agent" claim without inspectable state transitions and bounded tool budgets |
| ops/service | offline lane remains credential-free; FastAPI/Pydantic async/SSE service and report/dashboard show gate freshness, latency, token/cost estimate, trace/run lookup, and failure classification |
...
3. Benchmark hardening: 100-document metadata coverage, 30 hard-negative
   abstention questions, 30 section-labeled lookup questions, 20 cross-document
   comparison questions, 25 reviewed visual/table questions, and 30 paraphrase
   questions are now the default offline query contract; next finish the
   30-question visual/table target and same-dataset ablations.
...
6. Agent-engineering evidence UX: expose answer/citation/chunk/source/gate,
   agent state transitions, tool calls, checkpoint/HITL status, trace IDs,
   latency/cost, and failure evidence in a small service/dashboard surface.
...
1. **Latest source-first real gate was missing and is now resolved.** The old
   `artifacts/eval_real` evidence was produced against the older
   `artifacts/index_real` manifest (`chunk_count=286`, no parsed-source
   lineage). The current canonical real index now records `text_source=parsed`,
   `parse_manifest_path=artifacts/parsed_docs/manifest.jsonl`, and
   `chunk_count=16459`, and the current `artifacts/eval_real` run passes
   `rag_quality_complete=true` under `rfp-rag-real-v5`; this is now historical
   because v6 adds cross-document hard floors and lineage requirements.
2. **Evaluation set is improved but still incomplete for the final visual/table
   claim.** The current offline generated set now covers all 100 documents for
   metadata (`golden_metadata=400`) and includes 30 near-domain hard-negative
   abstention controls, 30 section-labeled lookup questions, 20 cross-document
   comparison questions, 25 reviewed visual/table questions, and 30 paraphrase
   questions (`query_set_counts.total=545`). The remaining benchmark shortfall
   is the last 5 reviewed visual/table questions plus same-dataset ablations
   before stronger retrieval/reranker claims.
...
5. **Gate status was too shallow.** `rfp_rag.gate_status` now validates contract
   ...
   approved source-first real refresh is now historical; v6 requires a new real
   run and should fail closed until it exists and meets cross-document floors.
...
| query_set_counts.cross_document | `20` |
| query_set_counts.golden_metadata | `400` |
| query_set_counts.paraphrase | `30` |
| query_set_counts.section_lookup | `30` |
| query_set_counts.visual_table | `25` |
| query_set_counts.total | `545` |
| recall@5 | `0.9864077669902913` |
| all_expected_docs_retrieved@5 | `0.974757281553398` |
| cross_document.recall@5 | `0.7` |
| cross_document.all_expected_docs_retrieved@5 | `0.4` |
| paraphrase.recall@5 | `1.0` |
| paraphrase.metadata_exact_match | `0.8666666666666667` |
| visual_table.recall@5 | `1.0` |
| visual_table.visual_evidence_hit_rate | `0.92` |
| mrr | `0.9841423948220064` |
| metadata_exact_match | `0.9395348837209302` |
| citation_validity | `0.9844660194174757` |
| section_hit_rate | `1.0` |
...
This is the current gate state after the v6 hardening and real-lane refresh.
`gate_status` should report `overall_ok=true`; if stale or lineage-mismatched
artifacts are introduced, it should fail closed instead of preserving the
portfolio claim.

Claims to avoid until additional evidence exists:
...
## 19. Real eval recovery artifacts

The first `rfp-rag-real-v5` candidate index build succeeded without replacing
canonical `artifacts/index_real`:

| field | value |
|---|---|
| candidate index | `artifacts/index_real_v5_candidate` |
| text_source | `parsed` |
| parse_manifest_path | `artifacts/parsed_docs/manifest.jsonl` |
| index_text_source_counts.parsed | `100` |
| chunk_count | `16459` |

An earlier candidate real evaluation was intentionally run against
`artifacts/eval_real_v5_candidate`, but it was interrupted after roughly 45
minutes because the process was blocked inside RAGAS faithfulness judge OpenAI
chat completion response handling. No final candidate `metrics.json` was
produced, so that candidate run was **not** a real quality pass. The canonical
`artifacts/eval_real` run recorded in section 10-Current is now the current real
quality evidence.

Operational change:

- `evaluate.py` now writes query-set files and `contract.json` before long
  model calls.
- It appends `eval_progress.jsonl` during answer generation and judge phases.
- It writes `predictions_unjudged_partial.jsonl` per answered query.
- It writes `predictions_unjudged.jsonl` after answer generation completes,
  before judge starts.
- It appends `predictions_judged_partial.jsonl` per judged case.

This makes long-running real/open evals inspectable and partially recoverable:
an interrupted judge no longer hides whether retrieval/generation completed.
Final gate files remain `metrics.json`, `predictions.jsonl`, `report.md`, and
`contract.json`; partial files are operational evidence, not a quality pass.

## 20. Bounded agent team operation

ADR-0013 records the adopted agent-team operating model. The project does not
claim autonomous 16-agent development. Instead, it uses a bounded,
human-guided model suitable for this repo's constraints:

- main integrator remains singular;
- implementation tasks should be split into disjoint write sets first;
- parallel writer subagents are allowed for those disjoint write sets;
- read-only reviewer/explorer subagents run in parallel for critique and
  verification planning;
- real/API commands, canonical artifact updates, dependency/architecture
  decisions, and shared-file edits stay with the main agent;
- every handoff must specify owner scope, forbidden scope, and return contract.

This incorporates the useful parts of the multi-agent compiler workflow
...
| field | value |
|---|---|
| local_evidence_bundle_check | `true` |
| portfolio_readiness_check | `true` |
| failed | `[]` |
| second_stage_readiness.complete | `true` |
| second_stage_readiness.missing | `[]` |
| stage2_contract_schema_enforced | `true` |
| deferred | `cloud_deployment`, `public_dashboard` |

Stage 2 scaffold command:

```bash
python3 -m rfp_rag.stage2_scaffold
```

This command creates the expected Stage 2 artifact paths and computes the
current offline eval-set coverage from `artifacts/eval/*.jsonl`. Current local
evidence is measured-pass across real frozen evidence, agent stress, retrieval bakeoff,
visual quality, service ops, deterministic security smoke, and cost artifacts.

Stage 2 service ops command:

```bash
uv run python -m rfp_rag.stage2_service_ops
```

Current local service ops artifact is measured-pass for the lightweight service
contract smoke: `/healthz`, `/v1/answer`, `/v1/answer/stream`, `/v1/gates`,
`/v1/ops/summary`, artifact path-safety rejection, latency recording, and
token/cost observability fields. The default command intentionally stubs the
answer and gate internals so this gate measures the FastAPI/Pydantic/SSE/ops
surface quickly; full offline answer and full gate-status execution remain
available with `--full-answer` and `--full-gates`.

Stage 2 deterministic security smoke command:

```bash
uv run python -m rfp_rag.security_redteam
```

Current local security artifact is measured-pass for deterministic
prompt-injection and secrets blocking, malicious document/retrieved-evidence/
tool-output fixture cases, tool allowlist and max tool-call budget enforcement,
audit redaction, publishable allowlist, retention scope, and selected
demo-visible artifact leak scan checks. It is not a full hosted-service
adversarial security audit or whole-repo leak scan.

Stage 2 agent stress command:

```bash
uv run python -m rfp_rag.agent_stress
```

Current local agent stress artifact is measured-pass for deterministic LangGraph
replays covering direct RAG, rewrite recovery, abstention, metadata-tool route,
HITL approve/reject convergence, no report side effect before approval,
thread-id reuse isolation, checkpointer close path, audit-argument redaction,
and ops-tool budget behavior. It does not claim full checkpoint-state redaction
or agent-level tool-call budgeting.

Stage 2 cost budget command:

```bash
uv run python -m rfp_rag.cost_budget
```

Current local cost-budget artifact is measured-pass for deterministic token and
cost estimates over persisted `artifacts/eval_real` and `artifacts/eval_open`
predictions. It also records optional slots for `eval_open_rerank`,
`eval_real_rerank`, and `eval_stage2_real`, so approved paid/API reranker or
Stage 2 real runs are included automatically once their artifacts exist. This is
a reproducible regression budget estimate, not provider billing evidence.

Paid/API lane dry-run plan command:

```bash
uv run python -m rfp_rag.paid_lane_plan
```

Current local paid-lane plan artifact is measured-pass as a dry-run checklist:
it records the cost-bearing `real_index_v6`, `real_eval_v6`,
`stage2_real_eval`, and `same_set_open_reranker_eval` commands, required
environment variable names, expected artifact writes, current cost-budget
estimate, and post-run `gate_status` / `portfolio_check` verification commands.
It deliberately does not execute provider calls.

Stage 2 real frozen-evidence finalize command:

```bash
uv run python -m rfp_rag.stage2_real
```

Current local real frozen-evidence artifact is measured-pass. The finalize command is
intentionally non-provider: it reads the persisted
`artifacts/eval_stage2_real/metrics.json`, attaches the frozen `eval_set_hash`,
maps aggregate metrics into the Stage 2 contract fields, and keeps
`holdout_quality_complete=false` unless lineage, `evaluation_valid`, and all
quality floors pass.

Stage 2 retrieval bakeoff command:

```bash
uv run python -m rfp_rag.retrieval_bakeoff
```

Current local retrieval bakeoff artifact is measured on the comparable modes.
It records available required modes `vector`, `bm25`, and `hybrid_rrf`. Those
artifacts are measured on the current 545-query offline set and share the same
comparison hash. Per ADR-0020 and ADR-0008, `reranker` remains an
optional/deferred mode until a same-set `reranker="llm"` artifact exists. The
approved paid/API attempt did not produce such evidence because the
`real_openai` path returned OpenAI `insufficient_quota`, while the `open` path
used a non-comparable 515-query set.

Stage 2 visual quality command:

```bash
uv run python -m rfp_rag.visual_quality
```

Current local visual-quality artifact is measured-pass. It records
`visual_question_count=30`, `visual_evidence_hit_rate=0.92`,
`unsupported_visual_claim_rate=0.0353`, and sidecar citation/abstention
no-regression metrics at `1.0`.
