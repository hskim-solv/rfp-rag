...
def _write_valid_portfolio_gates(root: Path) -> None:
    ...
    _write_jsonl(root / "artifacts/eval/predictions.jsonl", _records("offline", 70))
    _write_jsonl(root / "artifacts/eval_real/predictions.jsonl", _records("real", 70))
    ...
    _write_json(
        ...
        {
            "contract_version": "rfp-rag-real-v3",
            "required_commands": [
...
def test_collect_gate_status_validates_fresh_portfolio_artifacts(
    ...
    assert status["lanes"]["offline_rag"]["contract_version"] == "rfp-rag-offline-v2"
